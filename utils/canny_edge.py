# -*- coding: utf-8 -*-
"""
Created on Tue Dec  8 16:03:24 2020
...
def canny_edge_detector(image, sigma=1,low_threshold=50,high_threshold=100):
    """
    ...
    edges = feature.canny(image, sigma=sigma,low_threshold=low_threshold,high_threshold=high_threshold)  # TODO: the sigma hyper parameter value is not defined in the paper.
    size = edges.shape[::-1]
    data_bytes = np.packbits(edges, axis=1)
    edges = Image.frombytes(mode='1', size=size, data=data_bytes)
    return edges

def cannyEdge(img, sigma=1, th1=50, th2=80):
    """
...
if __name__ == '__main__': 
    ...
    img = Image.open('D:/tmp/img.jpg')
    edge = canny_edge_detector(img)
    output_image='D:/tmp'
    
    Image.Image.save(edge,os.path.join(output_image,'%s_%s_%s_edge2.jpg'%('1','50','100')))
    
    # img = io.imread('D:/tmp/img.jpg')
    # img = color.rgb2gray(img)
    # sigma=3
    # lower_thresh=50
    # upper_thresh=100
    # gauss, magnitude, weak, strong = cannyEdge(img, sigma, lower_thresh, upper_thresh)
    
    # io.imsave(os.path.join(output_image,'%s_%s_%s_gauss.jpg'%(sigma,lower_thresh,upper_thresh)), gauss)
    # io.imsave(os.path.join(output_image,'%s_%s_%s_magnitude.jpg'%(sigma,lower_thresh,upper_thresh)), magnitude)
    # io.imsave(os.path.join(output_image,'%s_%s_%s_weak.jpg'%(sigma,lower_thresh,upper_thresh)), weak)
    # io.imsave(os.path.join(output_image,'%s_%s_%s_strong.jpg'%(sigma,lower_thresh,upper_thresh)), strong)
